...
👔 Profil professionnel    : ${state.userJourney.profile ? state.userJourney.profile.replace('profile_', '').replace('_', ' ') : 'Non spécifié'}
🎯 Objectif principal      : ${state.userJourney.goal ? state.userJourney.goal.replace('goal_', '').replace('_', ' ') : 'Non spécifié'}
🔧 Services sélectionnés   : ${state.userJourney.services && state.userJourney.services.length > 0 
    ? state.userJourney.services.map(service => service.replace('service_', '').replace('_', ' ')).join(', ')
    : 'Non spécifié'}
📊 Méthode de recherche    : ${state.userJourney.method ? state.userJourney.method.replace('method_', '').replace('_', ' ') : 'Non spécifié'}
👥 Taille d'échantillon    : ${state.userJourney.sampleSize ? state.userJourney.sampleSize.replace('sample_', '').replace('_', ' ') : 'Non spécifié'}
⏰ Délai souhaité          : ${state.userJourney.timeline ? state.userJourney.timeline.replace('timeline_', '').replace('_', ' ') : 'Non spécifié'}
💰 Budget approximatif     : ${state.userJourney.budget ? state.userJourney.budget.replace('budget_', '').replace('_', ' ') : 'Non spécifié'}
🌐 Langue                  : ${state.currentLanguage.toUpperCase()} (${state.currentLanguage === 'fr' ? 'Français' : 'English'})
...
Cette demande a été générée automatiquement par le chatbot Omega
═══════════════════════════════════════════════════════════════
        `,
